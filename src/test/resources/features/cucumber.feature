...
Функция: Тестирование edujira.ifellow.ru
  ...
  Структура сценария: Авторизация
    ...
    Тогда Выполняются проверки работоспособности статуса задачи поле "<статус>" поле "<статусс>" поле "<статуссс>" поле "<статусссс>" поле "<статуссссс>" поле "<статусссссс>" поле "<статуссссссс>"
    Примеры:
      ...
      | AT16  | Qwerty123 | TestSelenium | Ошибка | TestSelenium | Шаги:  1.Тест вызванных функций  2.Проверка результатов тестирования | Version 2.0 | Medium    | bugBuzzers | local machine | Сортировать по Cоздано | Мои открытые задачи | РЕШЕННЫЕ | В РАБОТЕ  | ПЕРЕОТКРЫТ | ГОТОВО      | Готово       |